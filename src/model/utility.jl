...
function IUW5(θ,F,eW,AW,κ)
    ...
    U1 = α_C*log(max(b,exp(logY_W)/2)) + α_ℓ*log(92)
    U2 = α_C*log(max(b,exp(logY_W))) + α_ℓ*log(72)
    return (U0, U1, U2)
end
...
function IUW4(θ,F,edW,edH,AW,κ,AH,ϵH,AK)
    ...
    U1 = α_C*log(π_H*exp(logY_H) + exp(logY_W)/2) + (α_ℓ + αΓ_τWa[AK+1])*log(92)
    U2 = α_C*log(π_H*exp(logY_H) + exp(logY_W)) + (α_ℓ + αΓ_τWa[AK+1])*log(72)
    return (U0,U1,U2)
end

function IUH4(θ,F,edH,AH,ϵH,AK)
    (; α_C, α_ℓ, γ_YH, αΓ_τWa, αΓ_τHa) = θ
    (;π_H) = F
    ...
    U1 = U + αΓ_τWa[AK+1]*log(92)
    U2 = U + αΓ_τWa[AK+1]*log(72)
    return (U0, U1, U2)
end
...
function IU3(θ,eW,eH,AW,AH,ϵH,κ,ω)
    ...
    logY_H = γ_YH[eH,1] + γ_YH[eH,2]*AH + ϵH
    vω = α_ω[1] + α_ω[2]*ω
    uc1 = α_C*log(exp(logY_H) + exp(logY_W)/2)
    uc2 = α_C*log(exp(logY_H) + exp(logY_W))

    UW0 = α_C*logY_H + α_ℓ*log(112) + vω
    UW1 = uc1 + α_ℓ*log(72) + vω
    UW2 = uc2 + α_ℓ*log(72) + vω

    UH0 = α_C*logY_H + α_ℓ*log(72) + vω
    UH1 = uc1 + α_ℓ*log(72) + vω
    UH2 = uc2 + α_ℓ*log(72) + vω
...
function IU2(θ,eW,eH,AW,AH,ϵH,κ,ω,AK)
    ...
    u1 = αΓ_τHa[AK+1]*log(72) + αΓ_τWa[AK+1]*log(92)
    u2 = αΓ_τHa[AK+1]*log(72) + αΓ_τWa[AK+1]*log(72)
