...
lc_moms <- P %>%
  ...
  summarize(Divorced = mean(DIV),Kids = mean(YBIRTH<=YEAR),FT = mean(M_hrs>1500,na.rm = TRUE),PT = mean(M_hrs>0 & M_hrs<1500,na.rm = TRUE),DivKids = mean(DIV & YBIRTH<=YEAR)) %>%
  pivot_longer(-M_AGE)
...
P %>%
  filter(M_AGE==18) %>%
  select(MID,YEAR,YBIRTH,YMAR,YBIRTH_M) %>%
  filter(YEAR>=YBIRTH)

P %>%
  filter(M_AGE==18) %>%
  summarize(mean(YEAR>=YBIRTH))
